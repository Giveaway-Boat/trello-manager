...
module.exports = async (client) => {
    ...
    process.on('unhandledRejection', async (reason, promise) => {
        ...
        let errorLogsChannel = client.channels.cache.get(process.env.ERROR_LOGS_CHANNEL);
        if (!errorLogsChannel) errorLogsChannel = await client.channels.fetch(process.env.ERROR_LOGS_CHANNEL);
        const errEmbed = new EmbedBuilder().setColor('#000000').setTitle(`An Error Occured:`).setDescription(`\`\`\`${reason}\`\`\``).setTimestamp();
